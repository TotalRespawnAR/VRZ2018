...
Shader "RM/Holographic/LB Transparent CutOut" {
    Properties {
      ...
      _HolTile ("Hologram Tilling", Float) = 22
	  _HolOffSet ("Hologram Offset", Float) = -0.09
	  _EmP ("Hologram Power", Float) = 0.2

	  [MaterialToggle] Dissolve ("Dissolve Effect", Float) = 1

      _ColorEm ("Dissolve Color", Color) = (0,0,0,1)
      _DissolveTex ("Dissolve Texture (RGB)", 2D) = "Black" {}
      _Dis ("Dissolve", Range(0.0, 1.0)) = 1
      _EdjeSize ("Edje Size", Range(0.0, 1.0)) = 0.08
      _Emission ("Emission", Range(0.0, 50)) = 0


	[MaterialToggle] Deform ("Deform Mesh", Float) = 0

	_ShakeDisplacement ("Displacement", Range (-1, 10)) = 1.1
    _ShakeTime ("Shake Time", Range (-1, 10)) = 4
    _Shakespeed ("Shake Speed", Range (-1, 10)) = 1.3
    _ShakeBending ("Shake Bending", Range (-1, 1)) = -0.02

    [MaterialToggle] fresnel  ("Fresnel Effect", Float) = 1
    _RimPower ("Rim Power", Range(0,8.0)) = 5.1
