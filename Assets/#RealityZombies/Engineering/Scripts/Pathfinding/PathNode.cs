...
public class PathNode {
    ...
    public void setGP_toptext(float argval, bool argIstCheck)
    {
        if (argIstCheck)
        {
            GPofThisNode.SetTXTtop("T" + argval.ToString());
        }
        else
        {
  ...
  public bool applyCostTweek =false; //no longer needed if i make zombie god observer
    public PathNode(GameObject thisPoint, float segmentDistance, PathNode previousNode, float argcost)
    {
        ...
        if(applyCostTweek)   //was   if (GameSettings.Instance.applyCostTweek)
        {
