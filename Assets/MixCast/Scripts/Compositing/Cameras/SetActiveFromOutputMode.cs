    ...
    public class SetActiveFromOutputMode : CameraComponent
    {
        ...
        bool CalculateNewState()
        {
            ...
            for (int i = 0; i < matchModes.Count; i++)
            {
                switch(matchModes[i])
                {
                    case MixCastData.OutputMode.Immediate:
                        if (context.Data.bufferTime <= 0 && context.Data.outputMode != MixCastData.OutputMode.Quadrant)
                            return true;
                        break;
                    case MixCastData.OutputMode.Buffered:
                        if (context.Data.bufferTime > 0 && context.Data.outputMode != MixCastData.OutputMode.Quadrant)
                            return true;
                        break;
                    case MixCastData.OutputMode.Quadrant:
                        if (context.Data.outputMode == MixCastData.OutputMode.Quadrant)
                            return true;
                        break;
                }
            }
