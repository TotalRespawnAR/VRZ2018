...
namespace BlueprintReality.MixCast {
	...
	public class MixCastProjectSettingsInspector : Editor {
        ...
        void DrawRenderingGroup()
        {
            ...
            EditorGUILayout.LabelField("General", subHeaderStyle, GUILayout.Height(28));
            ...
            if( EditorGUI.EndChangeCheck() )
            {
                if( layerCamPrefabProp.objectReferenceValue != null )
                {
                    ...
                    else
                    {
                        ...
                        {
                            finalPassCamPrefabProp.objectReferenceValue = oldVal;
            ...
            oldVal = finalPassCamPrefabProp.objectReferenceValue;
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(finalPassCamPrefabProp);
            if (EditorGUI.EndChangeCheck())
            {
                if (finalPassCamPrefabProp.objectReferenceValue != null)
                {
                    Camera cam = (finalPassCamPrefabProp.objectReferenceValue as GameObject).GetComponent<Camera>();
                    if (cam == null)
                    {
                        finalPassCamPrefabProp.objectReferenceValue = oldVal;
                        EditorUtility.DisplayDialog("Error", "Camera Prefab must contain a Camera component for MixCast to use. Please correct the issue and try again", "OK");
